...
// The reusable month grid: header with month nav, the 6-week grid, a legend,
// and a selected-day detail card. No page chrome, so it can live inside a tab
// or inside the full-screen FamilyCalendar.
export default function MonthCalendar({ events, children, adminMode = false, onAdd, onEdit, eyebrow = 'Family calendar' }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-4">
        <div>
          {eyebrow && <div className="text-muted text-sm uppercase tracking-[0.22em]">{eyebrow}</div>}
          <h1 className="font-display text-3xl md:text-4xl text-ink mt-1">{monthLabel}</h1>
      ...
      <div className="grid grid-cols-7 gap-1.5 md:gap-2 mb-1.5">
        {DOW_HEADERS.map(d => (
          <div key={d} className="text-[11px] uppercase tracking-[0.18em] text-center py-1 text-muted">
            {d}
          </div>
        ))}
      </div>
              ...
              <div className="flex items-center justify-between">
                <span
                  className={`inline-flex items-center justify-center text-sm font-semibold ${
                    isToday ? 'bg-ink text-cream rounded-full w-6 h-6' : 'text-ink'
                  }`}
                >
                  {date.getDate()}
                </span>
